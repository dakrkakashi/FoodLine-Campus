...
async function GET(request, { params }) {
    ...
    const stream = new ReadableStream({
        ...
        `)
                ...
                .single();
            if (initialOrder) {
                controller.enqueue(encoder.encode(`data: ${JSON.stringify({ type: 'ORDER_SNAPSHOT', order: initialOrder })}\n\n`));
            }
            // 2. Realtime listener
            const channel = supabase
                ...
                controller.enqueue(encoder.encode(`data: ${JSON.stringify({ type: 'ORDER_UPDATE', order: payload.new })}\n\n`));
            })
                .subscribe();
            // Keepalive heartbeat
