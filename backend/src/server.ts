...
// -----------------------------------------------------------------------------
// 3. POST /api/orders (with order submission rate limiter & input validation)
// -----------------------------------------------------------------------------
app.post('/api/orders', orderPlacementLimiter, async (req: Request, res: Response) => {
  try {
...
app.post('/api/payments/verify-utr', utrRateLimiter, async (req: Request, res: Response) => {
  try {
    ...
    if (utrNumber && !SecurityValidators.isValidUtr(String(utrNumber))) {
      return res.status(400).json({
        success: false,
        error: 'Malformed UTR: Transaction reference must be exactly 12 numeric digits.',
      });
    }
  ...
  try {
    ...
    res.json({
      ...
      data: {
        ...
        status: order.status,
        paymentStatus: order.paymentStatus,
        message: 'Payment successfully reconciled and verified by staff.',
      },
      meta: { timestamp: new Date().toISOString() },
    });
  } catch (error: any) {
...
// -----------------------------------------------------------------------------
// 4b. POST /api/orders/verify-otp (with 5-attempt per 15 min brute-force guard)
// -----------------------------------------------------------------------------
app.post('/api/orders/verify-otp', otpRateLimiter, async (req: Request, res: Response) => {
  try {
    ...
    if (!SecurityValidators.isValidOtp(String(pickupOtp))) {
      return res.status(400).json({
        success: false,
        error: 'Malformed OTP: Pickup OTP must be exactly 4 numeric digits.',
      });
    }
...
// -----------------------------------------------------------------------------
// 6. PATCH /api/kds/orders/:id/status (Protected: kitchen, canteen_manager, admin)
// -----------------------------------------------------------------------------
app.patch('/api/kds/orders/:id/status', requireAuth(['kitchen', 'canteen_manager', 'admin']), async (req: Request<{ id: string }>, res: Response) => {
  try {
    const { id } = req.params;
...
// -----------------------------------------------------------------------------
// 7. PATCH /api/kds/inventory/:dishId (Protected: kitchen, canteen_manager, admin)
// -----------------------------------------------------------------------------
app.patch('/api/kds/inventory/:dishId', requireAuth(['kitchen', 'canteen_manager', 'admin']), async (req: Request<{ dishId: string }>, res: Response) => {
  try {
...
// -----------------------------------------------------------------------------
// 8. GET /api/admin/metrics (Protected: admin, canteen_manager)
// -----------------------------------------------------------------------------
app.get('/api/admin/metrics', requireAuth(['admin', 'canteen_manager']), async (req: Request, res: Response) => {
  try {
...
// -----------------------------------------------------------------------------
// 9. POST /api/admin/orders/cleanup (Protected: admin only)
// -----------------------------------------------------------------------------
app.post('/api/admin/orders/cleanup', requireAuth(['admin']), async (req: Request, res: Response) => {
  try {
