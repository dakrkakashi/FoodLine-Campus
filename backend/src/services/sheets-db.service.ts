...
export class SheetsDbService {
  ...
  /**
   * Hashes a password using cryptographically random 16-byte salt with scrypt (64-byte key).
  ...
  /**
   * Verifies password against salted scrypt hash or legacy SHA-256 hash.
   * Timing-attack protected using crypto.timingSafeEqual.
   * Plaintext comparison is strictly eliminated.
   */
  public static verifyPassword(inputPassword: string, storedHash: string): boolean {
    if (!inputPassword || !storedHash) return false;

    // 1. Salted scrypt verification ($scrypt$salt$hash)
    ...
    // 2. Backward compatibility: Legacy $sha256$ prefix from Google Apps Script trigger
    if (storedHash.startsWith('$sha256$')) {
      ...
      const bufB = Buffer.from(rawHash);
      if (bufA.length === bufB.length) {
        return crypto.timingSafeEqual(bufA, bufB);
      }
      return false;
    ...
    // Security hardening: reject unhashed plaintext passwords
    return false;
  }
  ...
  public static async appendStudentUser(user: {
    ...
    // Salted scrypt hash ($scrypt$salt$hash)
    const passHash = this.hashPassword(user.password);
    const rowValues = [
  ...
  public static async verifyUtr(
  ...
  ): Promise<{ valid: boolean; message: string; payment?: SheetPayment; requiresCounterCheck?: boolean }> {
    ...
    // but flag as requiring counter confirmation by staff
    return {
      valid: true,
      requiresCounterCheck: true,
      message: '12-digit UTR accepted. Awaiting visual confirmation at pickup counter.',
    };
  }
  ...
  public static async recordPayment(payment: SheetPayment): Promise<boolean> {
    ...
    const cleanPrn = payment.prn.trim().toUpperCase();
    const cleanEmail = `${cleanPrn.toLowerCase()}@sanjivani.edu.in`;
    ...
    try {
      await sheets.spreadsheets.values.append({
        spreadsheetId,
        range: "'FoodLine — Payment & UTR Form'!A:H",
        valueInputOption: 'USER_ENTERED',
        insertDataOption: 'INSERT_ROWS',
        requestBody: {
          values: [rowValues],
      ...
      console.error('[SheetsDbService] Error recording payment in FoodLine — Payment & UTR Form tab:', err?.message || err);
      return false;
    }
  }
