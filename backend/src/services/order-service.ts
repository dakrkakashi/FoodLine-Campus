...
export class OrderService {
  ...
  public static async confirmUtrPayment(
    ...
    order.paymentStatus = 'PENDING_MANUAL_REVIEW';
    order.payment_status = 'PENDING_MANUAL_REVIEW';
  ...
  /**
   * Reconcile payment status (Staff/Admin verified payment against soundbox or bank statement)
   */
  public static async reconcilePayment(orderToken: string, verifiedBy: string = 'Staff'): Promise<Order> {
    const order = await OrderService.getOrderByToken(orderToken);
    if (!order) {
      throw new Error(`Order ${orderToken} not found`);
    }
