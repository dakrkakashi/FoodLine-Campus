...
export function AuthProvider({ children }: { children: React.ReactNode }) {
      ...
      try {
        ...
        if (mounted) {
          ...
          if (currentUser) {
            ...
            if (savedStudent) {
              try { parsedStudent = JSON.parse(savedStudent); } catch {}
            }
            if (!parsedStudent && typeof document !== 'undefined') {
              const match = document.cookie.match(/foodline_student_session=([^;]+)/);
              if (match) {
                try {
                  parsedStudent = JSON.parse(decodeURIComponent(match[1]));
                  localStorage.setItem('foodline_student_session', JSON.stringify(parsedStudent));
                } catch {}
              }
            }
            ...
            } else if (parsedStudent && parsedStudent.prn) {
              setUser({
                id: parsedStudent.id || `student_${parsedStudent.prn}`,
                email: parsedStudent.email || `student_${parsedStudent.prn}@sanjivani.edu.in`,
                user_metadata: {
                  full_name: parsedStudent.full_name,
                  prn: parsedStudent.prn,
                  phone: parsedStudent.phone,
                  role: 'student',
                },
              } as unknown as User);
              setProfile(parsedStudent);
              localStorage.setItem('foodline_last_prn', parsedStudent.prn);
              if (parsedStudent.full_name) {
                localStorage.setItem('foodline_last_name', parsedStudent.full_name);
              }
            }
          }
        }
  ...
  const signInWithPrnPassword = useCallback(async (prn: string, password: string) => {
    ...
    try {
      ...
      if (typeof window !== 'undefined') {
        localStorage.setItem('foodline_student_session', JSON.stringify(studentProf));
        localStorage.setItem('foodline_last_prn', cleanPrn);
        if (json.student?.full_name) {
          localStorage.setItem('foodline_last_name', json.student.full_name);
        }
      }
      if (typeof document !== 'undefined') {
        const cookieVal = encodeURIComponent(JSON.stringify(studentProf));
  ...
  const signUpWithPrnPassword = useCallback(async (prn: string, password: string, fullName: string, email?: string) => {
    ...
    try {
      const res = await fetch('/api/auth/student-signup', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ prn: cleanPrn, password, fullName, email: cleanEmail }),
      });
      const json = await res.json();
      if (!res.ok || !json.success) {
      ...
      const studentEmail = json.student?.email || cleanEmail || `student_${cleanPrn}@sanjivani.edu.in`;
      ...
      if (json.session?.access_token && json.session?.refresh_token) {
        ...
        const studentUser = {
          ...
          email: studentEmail,
          user_metadata: {
            full_name: json.student?.full_name || fullName,
            prn: cleanPrn,
            role: 'student',
          },
        } as unknown as User;
      ...
      if (typeof window !== 'undefined') {
        localStorage.setItem('foodline_student_session', JSON.stringify(studentProf));
        localStorage.setItem('foodline_last_prn', cleanPrn);
        if (json.student?.full_name || fullName) {
          localStorage.setItem('foodline_last_name', json.student?.full_name || fullName);
        }
      }
      if (typeof document !== 'undefined') {
        const cookieVal = encodeURIComponent(JSON.stringify(studentProf));
