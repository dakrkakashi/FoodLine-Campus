 ...
 * Specially calibrated for university canteen acoustics (Cafe @7, Sanjivani University).
 */

import { SoundSettings } from './types';

export type AnnouncementLanguage = 'en' | 'hi' | 'mr';
...
/**
 * 📣 Announce order ready at Counter with multilingual support
 */
...
/**
 * Modern Object-oriented wrapper for KDS and Custom Components
 */
export class VoiceAnnouncer {
  ...
  public static announceOrderReady(
    token: string,
    options: AnnouncementOptions = {}
  ): Promise<boolean> {
    const langMap: Record<AnnouncementLanguage, 'en-IN' | 'hi-IN' | 'mr-IN'> = {
      en: 'en-IN',
      hi: 'hi-IN',
      mr: 'mr-IN',
    };
    const settings: SoundSettings = {
      ...
      lang: langMap[options.lang || 'mr'],
    };
    announceOrderReady(token, 1, false, settings);
