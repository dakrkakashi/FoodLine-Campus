      ...
      <main className="max-w-3xl mx-auto px-4 pt-6 relative z-10">
        ...
        <div className="flex items-center justify-between gap-4 mb-6">
          <div className="flex items-center gap-2">
            <Link
              href="/menu"
              className="text-xs font-bold text-(--text-secondary) hover:text-(--text-primary) px-3.5 py-2 rounded-xl bg-black/5 dark:bg-white/5 border border-(--border-glass) flex items-center gap-2 transition cursor-pointer"
            >
              <ArrowLeft size={14} />
            ...
            <Link
              href="/orders"
              className="text-xs font-bold text-(--accent-orange) hover:text-(--text-primary) px-3.5 py-2 rounded-xl bg-(--accent-orange)/10 hover:bg-(--accent-orange)/20 border border-(--accent-orange)/20 flex items-center gap-2 transition cursor-pointer"
            >
              <Receipt size={14} />
              <span>My Orders</span>
            </Link>
          ...
          <div className="flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-black/5 dark:bg-white/5 border border-(--border-glass) backdrop-blur-md">
            <span
        ...
        <SpotlightCard
          ...
          className={`p-6 md:p-8 rounded-[2.5rem] border-2 shadow-2xl mb-8 transition-all duration-700 ${
            isReady
              ? 'bg-linear-to-b from-[#00D4AA]/20 via-(--bg-card) to-(--bg-card) border-[#00D4AA]/70 shadow-[#00D4AA]/30'
              : isCollected
              ? 'bg-linear-to-b from-zinc-500/10 via-(--bg-card) to-(--bg-card) border-(--border-glass) opacity-85'
              : 'bg-linear-to-b from-(--accent-orange)/15 via-(--bg-card) to-(--bg-card) border-(--accent-orange)/40 shadow-xl'
          }`}
        >
          {/* Header */}
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-(--border-glass)">
            <div>
              <div
            ...
            <div className="relative overflow-hidden text-left sm:text-right bg-black/5 dark:bg-white/5 border border-(--border-glass) rounded-2xl p-4 backdrop-blur-md shadow-sm">
              <BorderBeam
                size={140}
                duration={8}
                ...
                borderWidth={2}
              />
              <div className="text-[10px] font-bold text-(--text-secondary) uppercase tracking-wider mb-0.5">Pickup OTP</div>
              <div
                className={`text-4xl font-black font-mono tracking-widest ${
                  isCollected ? 'text-(--text-muted) line-through' : isReady ? 'text-[#00D4AA] animate-pulse' : 'text-(--text-primary)'
                }`}
              >
                {order?.pickup_otp || '----'}
              </div>
          ...
          {/* Slot Info */}
          {order?.pickup_slots && (
            <div className="mt-2 bg-black/3 dark:bg-white/5 backdrop-blur-md border border-(--border-glass) rounded-2xl p-4 flex items-center justify-between">
              <div className="flex items-center gap-3">
                <span className="text-2xl">⏱️</span>
                <div>
                  <div className="text-[10px] text-(--text-secondary) font-bold uppercase tracking-wider mb-0.5">
                    Reserved Break Slot
                  </div>
                  <div className="text-sm font-black text-(--text-primary)">{order.pickup_slots.label}</div>
                </div>
              </div>
              <span
          ...
          {/* Digital Thermal Slip Action Button */}
          <div className="mt-4 pt-3 border-t border-(--border-glass) flex items-center justify-between">
            <span className="text-xs text-(--text-secondary) font-medium">Need proof or offline pass?</span>
            <button
              onClick={() => setShowReceiptModal(true)}
              className="px-3.5 py-1.5 rounded-xl bg-black/5 dark:bg-white/10 hover:bg-black/10 dark:hover:bg-white/15 border border-(--border-glass) text-(--text-primary) text-xs font-extrabold flex items-center gap-1.5 cursor-pointer transition active:scale-95"
            >
        ...
        <SpotlightCard className="p-6 md:p-8 rounded-[2.5rem] bg-(--bg-card) border border-(--border-glass) shadow-2xl mb-8 relative overflow-hidden">
          <div className="flex items-center justify-between gap-4 mb-6">
            <h2 className="text-base font-black text-(--text-primary) flex items-center gap-2.5">
              <Utensils size={18} className="text-[#FF6B2C]" />
              <span>Live Kitchen Status Tracker</span>
            </h2>
            <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-black/5 dark:bg-white/5 border border-(--border-glass) text-xs font-bold text-(--text-secondary)">
          ...
          <div className="py-2 flex justify-center border-b border-(--border-glass) mb-6">
            {isReady ? (
              ...
              return (
                <div key={step.key} className="flex items-start gap-4 relative">
                  ...
                  {index < steps.length - 1 && (
                    <div
                      className={`absolute left-5 top-10 -bottom-5 w-0.5 rounded-full transition-colors duration-500 ${
                  ...
                  <motion.div
                    ...
                    className={`w-10 h-10 rounded-2xl flex items-center justify-center text-lg z-10 transition-all duration-500 shadow-md ${
                      isCurrent
                        ? 'bg-linear-to-tr from-accent-orange to-accent-amber text-white shadow-lg shadow-accent-orange/40 ring-2 ring-accent-orange ring-offset-2 ring-offset-(--bg-canvas)'
                        : isPassed
                        ? 'bg-[#00D4AA] text-black shadow-[#00D4AA]/20'
                        : 'bg-black/5 dark:bg-white/5 border border-(--border-glass) text-(--text-muted)'
                    }`}
                  >
                  ...
                  <div className="flex-1 min-w-0 pt-1">
                    <div className="flex items-center gap-2">
                      <h3
                        className={`text-sm md:text-base font-extrabold transition-colors duration-500 ${
                          isCurrent ? 'text-accent-amber' : isPassed ? 'text-(--text-primary)' : 'text-(--text-muted)'
                        }`}
                      >
                        {step.label}
                      </h3>
                    ...
                    <p className="text-xs text-(--text-secondary) mt-0.5 font-medium">{step.desc}</p>
                  </div>
                </div>
              );
            })}
        ...
        <div className="bg-(--bg-card) border border-(--border-glass) rounded-3xl p-6 shadow-xl space-y-4">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 pb-4 border-b border-(--border-glass)">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-xs font-black text-(--text-primary)">Cafe @7 FSSAI Certified Pure Veg Kitchen</div>
                <div className="text-[10px] text-(--text-secondary)">License #11522036000142 • Freshly cooked in 60-order break batches</div>
              </div>
            </div>
            <span className="text-[10px] uppercase font-black px-2.5 py-1 rounded-lg bg-black/5 dark:bg-white/5 text-(--text-secondary) border border-(--border-glass)">
        ...
        {/* Authentic Digital Thermal Receipt Slip Modal */}
        <ThermalReceiptModal
