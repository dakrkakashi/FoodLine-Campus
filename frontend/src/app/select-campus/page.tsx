    ...
    <PageTransition className="min-h-screen bg-(--bg-canvas) text-(--text-primary) pb-32 relative overflow-hidden transition-colors duration-500">
      ...
      <main className="max-w-5xl mx-auto px-4 pt-6 relative z-10">
        ...
        >
          ...
          <h1 className="text-3xl sm:text-5xl font-black text-(--text-primary) tracking-tight mb-3">
            Select Your{' '}
          ...
          <div className="mt-6 max-w-md mx-auto relative">
            ...
            <input
              ...
              className="w-full bg-(--bg-card) border border-(--border-glass) rounded-2xl pl-11 pr-4 py-3.5 text-sm text-(--text-primary) placeholder-(--text-muted) focus:outline-none focus:border-accent-orange focus:ring-2 focus:ring-accent-orange/20 transition-all shadow-xl backdrop-blur-xl"
            />
          </div>
        </motion.div>
        ...
        {globalSearch.trim() ? (
          <div className="space-y-4">
            ...
            {searchResults.length === 0 ? (
              <SpotlightCard className="p-10 rounded-[2.5rem] text-center border-dashed border-(--border-glass)">
                <GraduationCap className="w-12 h-12 mx-auto text-(--text-muted) mb-3" />
                <h3 className="text-lg font-bold text-(--text-primary) mb-1">No colleges found for "{globalSearch}"</h3>
                <p className="text-xs text-(--text-secondary) mb-6">Try searching by town name (e.g. Kopargaon, Pune) or browse the directory below.</p>
                <button
                  onClick={() => setGlobalSearch('')}
                  className="px-5 py-2.5 rounded-xl bg-black/5 dark:bg-white/10 hover:bg-black/10 dark:hover:bg-white/15 text-(--text-primary) text-xs font-bold transition cursor-pointer"
                >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {searchResults.map((campus) => (
                  <SpotlightCard
                    ...
                    className="p-6 rounded-4xl border border-(--border-glass) hover:border-accent-orange/50 hover:shadow-[0_0_30px_var(--accent-orange-glow)] transition-all cursor-pointer group flex flex-col justify-between bg-(--bg-card) shadow-sm dark:shadow-none"
                  ...
                  >
                    <div>
                      ...
                      <h3 className="text-lg font-black text-(--text-primary) group-hover:text-accent-amber transition-colors leading-snug mb-1">
                        {campus.name}
                      </h3>
                    ...
                    <div className="mt-5 pt-3 border-t border-(--border-glass) flex items-center justify-between">
          ...
          <div className="space-y-8">
            ...
            <div>
              <div className="flex items-center gap-2 mb-3">
                ...
                <h2 className="text-xs font-black uppercase tracking-wider text-(--text-primary)">Select State</h2>
              </div>
                  ...
                  <button
                    ...
                    className={`px-5 py-3 rounded-2xl text-xs font-black tracking-wider transition-all cursor-pointer flex items-center gap-2 ${
                      selectedStateId === st.id
                        ? 'bg-linear-to-r from-accent-orange to-accent-amber text-white shadow-lg shadow-accent-orange/25 border border-white/20'
                        : 'bg-(--bg-card) border border-(--border-glass) text-(--text-secondary) hover:text-(--text-primary) hover:border-(--accent-orange)/30'
                    }`}
                  >
                    <span>{st.name}</span>
                    ...
                    <button
                      ...
                      className={`px-5 py-3 rounded-2xl text-xs font-black tracking-wider transition-all cursor-pointer flex items-center gap-2 ${
                        selectedCityId === ct.id
                          ? 'bg-linear-to-r from-[#FFB347] to-[#FF6B2C] text-white shadow-lg shadow-[#FFB347]/25 border border-white/20'
                          : 'bg-(--bg-card) border border-(--border-glass) text-(--text-secondary) hover:text-(--text-primary) hover:border-(--accent-orange)/30'
                      }`}
                    >
                      <span>📍 {ct.name}</span>
            ...
            <div className="pt-4 border-t border-(--border-glass)">
              ...
              {drilledCampuses.length === 0 ? (
                <SpotlightCard className="p-8 rounded-3xl text-center border-dashed border-(--border-glass) bg-(--bg-card)">
                  <p className="text-xs text-(--text-secondary)">No campuses registered in this town yet.</p>
                </SpotlightCard>
              ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {drilledCampuses.map((campus) => {
                    ...
                    return (
                      <SpotlightCard
                        ...
                        className={`p-6 rounded-[2.5rem] border transition-all cursor-pointer group flex flex-col justify-between bg-(--bg-card) ${
                          isCurrent
                            ? 'border-[#FF6B2C] shadow-[0_0_35px_rgba(255,107,44,0.25)]'
                            : 'border-(--border-glass) hover:border-[#FF6B2C]/50 hover:shadow-[0_0_30px_rgba(255,107,44,0.18)]'
                        }`}
                        onClick={() => handleSelectCampus(campus)}
                      >
                        <div>
                          <div className="flex items-center justify-between gap-2 mb-3">
                            ...
                            <span className="text-[11px] font-bold text-(--text-secondary) flex items-center gap-1 bg-black/5 dark:bg-white/5 px-2.5 py-1 rounded-xl">
                              <Store size={13} className="text-[#FFB347]" /> {campus.totalCanteens || 5} Canteens Active
                            </span>
                          </div>

                          <h3 className="text-xl font-black text-(--text-primary) group-hover:text-[#FFB347] transition-colors leading-snug mb-1.5">
                            {campus.name}
                          </h3>
                          <p className="text-xs text-(--text-secondary) flex items-center gap-1.5 mb-2">
                            <MapPin size={13} className="text-[#FF6B2C] shrink-0" /> {campus.location}
                          </p>
                        </div>

                        <div className="mt-6 pt-4 border-t border-(--border-glass) flex items-center justify-between">
                          <span className="text-[11px] font-bold text-(--text-secondary)">
                            Pincode: <strong className="font-mono text-(--text-primary)">{campus.pincode || '423603'}</strong>
                          </span>
                          <motion.button
                            whileHover={{ scale: 1.05 }}
                            whileTap={{ scale: 0.95 }}
                            className="px-5 py-2.5 rounded-2xl bg-linear-to-r from-accent-orange to-accent-amber text-white font-black text-xs shadow-lg shadow-accent-orange/30 flex items-center gap-1.5"
                          >
