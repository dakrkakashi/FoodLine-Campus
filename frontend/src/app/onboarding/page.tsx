        ...
        <AnimatePresence mode="wait">
          ...
          >
            <SpotlightCard
              ...
              className="p-8 sm:p-10 rounded-[2.5rem] border border-(--border-glass) bg-(--bg-card) backdrop-blur-2xl shadow-2xl text-center relative overflow-hidden"
            >
              {/* Top Accent Radial */}
              <div
              ...
              {/* Big Icon */}
              <div className="w-20 h-20 mx-auto rounded-3xl bg-black/5 dark:bg-white/5 border border-(--border-glass) flex items-center justify-center text-4xl shadow-inner mb-6">
                {slide.icon}
              </div>

              {/* Badge */}
              <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-black/5 dark:bg-white/5 border border-(--border-glass) text-[10px] font-black uppercase tracking-widest text-[#FFB347] mb-3">
                <Sparkles size={11} />
                <span>{slide.badge}</span>
              </div>

              {/* Heading & Copy */}
              <h1 className="text-2xl sm:text-3xl font-black text-(--text-primary) tracking-tight leading-snug mb-3">
                {slide.title}
              </h1>
              <p className="text-xs sm:text-sm text-(--text-secondary) leading-relaxed max-w-md mx-auto mb-8 font-medium">
                {slide.description}
              </p>

              {/* Metric Callout */}
              <div className="p-4 rounded-2xl bg-black/5 dark:bg-white/5 border border-(--border-glass) max-w-xs mx-auto mb-8 shadow-inner">
                <div className="text-2xl sm:text-3xl font-black font-mono text-(--text-primary) tracking-tight" style={{ color: slide.accent }}>
                  {slide.metric}
                </div>
                <div className="text-[10px] uppercase font-bold text-(--text-secondary) tracking-wider mt-0.5">
                  {slide.metricLabel}
                </div>
              </div>
              ...
              <div className="space-y-3">
                ...
                <div className="pt-2 flex items-center justify-between gap-4 text-xs font-bold text-(--text-secondary)">
                  <Link href="/login" className="hover:text-(--text-primary) transition flex items-center gap-1">
                    <GraduationCap size={14} className="text-[#00D4AA]" />
                    <span>Student PRN Login</span>
                  </Link>

                  <Link href="/select-campus" className="hover:text-(--text-primary) transition flex items-center gap-1">
