...
export default function OrdersHistoryPage() {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      default:
        return {
          label: status,
          color: 'bg-black/5 dark:bg-white/5 text-(--text-secondary) border-(--border-glass)',
          icon: <Clock size={14} />,
        };
    }
  };
  ...
  return (
    <PageTransition className="min-h-screen bg-(--bg-canvas) text-(--text-primary) pb-28 transition-colors duration-500">
      <Navbar />
      ...
      <main className="max-w-4xl mx-auto px-4 pt-6 sm:pt-10">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
          <div>
            <div className="flex items-center gap-2 mb-1.5">
              <span className="px-2.5 py-0.5 rounded-full bg-(--accent-orange)/10 border border-(--accent-orange)/20 text-(--accent-orange) text-[10px] font-black tracking-wider uppercase">
                Student Portal
              </span>
              {activeOrdersCount > 0 && (
            ...
            <h1 className="text-3xl sm:text-4xl font-black text-(--text-primary) tracking-tight flex items-center gap-3">
              <Receipt className="text-(--accent-orange)" size={32} />
              My Orders & History
            </h1>
          ...
          <button
            ...
            className="self-start sm:self-center px-4 py-2.5 rounded-2xl bg-black/5 dark:bg-white/5 hover:bg-black/10 dark:hover:bg-white/10 border border-(--border-glass) text-xs font-bold text-(--text-secondary) hover:text-(--text-primary) transition flex items-center gap-2 cursor-pointer shadow-sm disabled:opacity-50"
          >
        ...
        <div className="flex flex-col sm:flex-row items-stretch sm:items-center justify-between gap-3 mb-6">
          {/* Tabs */}
          <div className="flex items-center p-1 rounded-2xl bg-black/5 dark:bg-white/5 border border-(--border-glass)">
            <button
              onClick={() => {
                setActiveTab('all');
                playTab();
              ...
              className={`flex-1 sm:flex-none px-4 py-2 rounded-xl text-xs font-black transition cursor-pointer ${
                activeTab === 'all'
                  ? 'bg-(--accent-orange) text-white dark:text-black shadow-md shadow-(--accent-orange)/20'
          ...
          <div className="relative flex-1 sm:max-w-xs">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 rounded-2xl bg-black/5 dark:bg-white/5 border border-(--border-glass) text-xs text-(--text-primary) placeholder-(--text-muted) focus:outline-none focus:border-(--accent-orange) transition"
            />
            {searchQuery && (
              <button
        ...
        {isLoading ? (
          <div className="py-20 text-center">
            <RefreshCw size={28} className="animate-spin text-(--accent-orange) mx-auto mb-3" />
            <p className="text-sm text-(--text-secondary) font-bold">Loading your order history...</p>
          </div>
        ) : filteredOrders.length === 0 ? (
          <SpotlightCard className="p-12 text-center rounded-[2.5rem] border-dashed border-(--border-glass) my-6">
            <div className="w-20 h-20 mx-auto rounded-3xl bg-black/5 dark:bg-white/5 border border-(--border-glass) flex items-center justify-center text-4xl mb-4 shadow-inner">
              🥡
            </div>
            ...
            <Link
              href="/menu"
              className="inline-flex items-center gap-2 px-6 py-3 rounded-2xl bg-linear-to-r from-(--accent-orange) to-(--accent-amber) text-white font-black text-xs uppercase tracking-wider shadow-xl shadow-(--accent-orange)/25 hover:brightness-110 transition cursor-pointer"
            >
              <Utensils size={16} />
          ...
          <div className="space-y-4">
            {filteredOrders.map((order, idx) => {
              ...
              return (
                ...
                >
                  <SpotlightCard
                    className={`p-5 sm:p-6 rounded-4xl border transition-all duration-300 ${
                      isActive
                        ? 'border-(--accent-orange)/40 bg-(--bg-card) shadow-xl shadow-(--accent-orange)/5'
                        : 'border-(--border-glass) bg-(--bg-card) hover:border-(--accent-orange)/30'
                    }`}
                  >
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-(--border-glass)">
                      {/* Token & OTP Info */}
                      <div className="flex items-center gap-3">
                        <div className="w-12 h-12 rounded-2xl bg-black/5 dark:bg-white/5 border border-(--border-glass) flex items-center justify-center text-xl shadow-inner font-black text-(--accent-orange)">
                          🏷
                        </div>
                        <div>
                    ...
                    <div className="py-4 grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                      {/* Item List */}
                      <div>
                        <div className="text-[10px] uppercase font-black tracking-wider text-(--text-muted) mb-2">
                          Ordered Dishes ({order.items?.length || 0})
                        </div>
                        <ul className="space-y-1.5">
                          {order.items?.map((item, iIdx) => (
                            <li key={iIdx} className="flex items-center justify-between text-(--text-primary)">
                              <span className="flex items-center gap-1.5">
                                <span className="w-1.5 h-1.5 rounded-full bg-(--accent-orange)"></span>
                                <strong className="text-(--text-primary) font-bold">{item.quantity}x</strong> {item.name}
                              </span>
                      ...
                      {/* Slot & Break Details */}
                      <div className="sm:border-l sm:border-(--border-glass) sm:pl-4 flex flex-col justify-between">
                        <div>
                          <div className="text-[10px] uppercase font-black tracking-wider text-(--text-muted) mb-2">
                            Break Pickup Slot
                          </div>
                          <div className="flex items-center gap-2 text-(--text-primary) font-bold bg-black/3 dark:bg-white/5 p-2.5 rounded-xl border border-(--border-glass)">
                            <Clock size={15} className="text-(--accent-teal)" />
                            <div>
                              <div>{order.slotLabel || 'Break Slot'}</div>
                              {order.slotTime && (
                                <div className="text-[10px] text-(--text-secondary) font-mono">{order.slotTime}</div>
                              )}
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="flex items-center justify-between pt-3 mt-2 border-t border-(--border-glass)">
                    ...
                    {/* Card Footer Actions */}
                    <div className="pt-3 border-t border-(--border-glass) flex flex-wrap items-center justify-between gap-3">
                      {/* Optical Pass Button */}
                      <Link
                        ...
                        className={`px-4 py-2.5 rounded-xl text-xs font-black transition flex items-center gap-2 cursor-pointer ${
                          isActive
                            ? 'bg-linear-to-r from-(--accent-orange) to-(--accent-amber) text-white shadow-lg shadow-(--accent-orange)/20 hover:brightness-110'
                            : 'bg-black/5 dark:bg-white/5 hover:bg-black/10 dark:hover:bg-white/10 text-(--text-primary) border border-(--border-glass)'
                        }`}
                      >
                        <QrCode size={15} />
                      ...
                      <button
                        ...
                        className="px-4 py-2.5 rounded-xl bg-black/5 dark:bg-white/5 hover:bg-black/10 dark:hover:bg-white/10 border border-(--border-glass) text-(--text-secondary) hover:text-(--text-primary) text-xs font-bold transition flex items-center gap-2 cursor-pointer"
                      >
                        <RotateCw size={14} className="text-(--accent-teal)" />
