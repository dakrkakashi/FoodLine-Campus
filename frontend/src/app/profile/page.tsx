...
export default function ProfilePage() {
  ...
  return (
    <PageTransition className="min-h-screen bg-(--bg-canvas) text-(--text-primary) transition-colors duration-500 pb-20">
      <Navbar />
      ...
      <main className="max-w-4xl mx-auto px-4 py-6 sm:py-10 space-y-6 sm:space-y-8">
        ...
        <motion.div
          ...
          className="relative overflow-hidden rounded-3xl p-5 sm:p-8 bg-(--bg-card) border border-(--border-glass) shadow-xl backdrop-blur-2xl"
        >
          {/* Subtle Ambient Glow */}
          ...
          <div className="relative z-10 flex flex-col sm:flex-row items-start sm:items-center gap-5 sm:gap-6 justify-between">
            <div className="flex items-center gap-4 sm:gap-5">
              ...
              <div className="relative">
                ...
                <span className="absolute -bottom-1 -right-1 w-5 h-5 rounded-full bg-emerald-500 border-2 border-(--bg-card) flex items-center justify-center" title="Active Campus Session">
              ...
              <div className="space-y-1">
                ...
                <div className="flex items-center gap-2 text-xs text-(--text-secondary)">
                  ...
                  <span className="truncate max-w-50 sm:max-w-none">{studentEmail}</span>
                </div>

                <div className="flex items-center gap-2 pt-0.5">
                  <button
                    onClick={copyPrn}
                    className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-black/5 dark:bg-white/5 hover:bg-black/10 dark:hover:bg-white/10 border border-(--border-glass) text-[11px] font-mono font-bold text-accent-teal transition cursor-pointer"
                  ...
                  >
                    ...
                    {copiedPrn ? (
                      ...
                      <span className="text-[9px] text-(--text-muted) uppercase tracking-widest">Copy</span>
                    )}
                  </button>
                </div>
              </div>
            ...
            <div className="flex items-center gap-2.5 w-full sm:w-auto pt-2 sm:pt-0">
              <Link
                href="/orders"
                className="flex-1 sm:flex-initial inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-black/5 dark:bg-white/10 hover:bg-black/10 dark:hover:bg-white/15 border border-(--border-glass) text-xs font-bold text-(--text-primary) transition active:scale-95 cursor-pointer shadow-xs"
              >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <motion.div
            ...
            className="rounded-3xl p-5 sm:p-6 bg-(--bg-card) border border-(--border-glass) space-y-5 shadow-xs"
          >
            <div className="flex items-center justify-between border-b border-(--border-glass) pb-3">
              <div className="flex items-center gap-2 text-sm font-black text-(--text-primary) uppercase tracking-wider">
            ...
            <div className="space-y-3">
              <div className="p-4 rounded-2xl bg-black/3 dark:bg-white/5 border border-(--border-glass) space-y-1">
                ...
                <p className="text-xs text-(--text-secondary)">{selectedCampus?.location || 'Post Box No 1, Sanjivani Factory Road'}</p>
              </div>
              ...
              <div className="p-4 rounded-2xl bg-black/3 dark:bg-white/5 border border-(--border-glass) space-y-1">
                ...
                <p className="text-xs text-(--text-secondary)">Express pickup counter with automated 60-order capacity limits.</p>
              </div>
            </div>
            ...
            <div className="p-3.5 rounded-2xl bg-linear-to-r from-emerald-500/10 to-teal-500/5 border border-emerald-500/20 flex items-center gap-3">
              ...
              <div className="text-[11px] text-(--text-secondary) leading-snug">
                Show your <strong className="text-(--text-primary)">4-Digit Pickup OTP</strong> or order token at the counter for contactless express handoff.
              </div>
            </div>
          </motion.div>
          ...
          <motion.div
            ...
            className="rounded-3xl p-5 sm:p-6 bg-(--bg-card) border border-(--border-glass) space-y-5 shadow-xs"
          >
            <div className="flex items-center gap-2 text-sm font-black text-(--text-primary) uppercase tracking-wider border-b border-(--border-glass) pb-3">
            ...
            <div className="space-y-2">
              <label className="text-xs font-bold text-(--text-secondary)">Dietary Filter Preference</label>
                  ...
                  <button
                    ...
                    className={`p-2.5 rounded-xl border text-xs font-bold transition flex flex-col items-center gap-1 cursor-pointer ${
                      dietaryPref === d.id
                        ...
                        : 'bg-black/3 dark:bg-white/5 border-(--border-glass) text-(--text-secondary) hover:text-(--text-primary)'
                    }`}
                  >
            ...
            {/* Sound FX Settings */}
            <div className="flex items-center justify-between p-3.5 rounded-2xl bg-black/3 dark:bg-white/5 border border-(--border-glass)">
              <div className="space-y-0.5">
                <div className="text-xs font-bold text-(--text-primary) flex items-center gap-1.5">
                  {muted ? <VolumeX size={14} className="text-(--text-muted)" /> : <Volume2 size={14} className="text-emerald-500" />}
                  <span>Web Audio Sound FX</span>
                </div>
                <p className="text-[11px] text-(--text-secondary)">Order chimes, button clicks, and kitchen notifications.</p>
              </div>
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-xl text-xs font-bold transition border cursor-pointer ${
                  muted
                    ? 'bg-black/5 dark:bg-white/5 text-(--text-muted) border-(--border-glass)'
            ...
            <div className="flex items-center justify-between p-3.5 rounded-2xl bg-black/3 dark:bg-white/5 border border-(--border-glass)">
              <div className="space-y-0.5">
                ...
                <p className="text-[11px] text-(--text-secondary)">Switch between Sunlit Day and OLED Night modes.</p>
              </div>

              <div className="flex items-center gap-1 p-1 rounded-xl bg-black/5 dark:bg-black/30 border border-(--border-glass)">
                <button
                  onClick={() => { setMode('light'); playClick(); }}
                  className={`px-3 py-1 rounded-lg text-xs font-bold transition cursor-pointer ${
                    mode === 'light' ? 'bg-white text-black font-black shadow-xs' : 'text-(--text-muted) hover:text-(--text-primary)'
                  }`}
                >
                  ☀️ Day
                </button>
                <button
                  ...
                  className={`px-3 py-1 rounded-lg text-xs font-bold transition cursor-pointer ${
                    mode === 'dark' ? 'bg-zinc-800 text-white font-black shadow-xs border border-white/10' : 'text-(--text-muted) hover:text-(--text-primary)'
                  }`}
                >
                  🌙 Night
                </button>
                  ...
                  <button
                    ...
                    className={`p-2 rounded-xl border text-xs font-bold transition flex items-center justify-center gap-1.5 cursor-pointer ${
                      theme === t.id
                        ...
                        : 'bg-black/3 dark:bg-white/5 border-(--border-glass) text-(--text-secondary) hover:text-(--text-primary) hover:bg-black/5'
                    }`}
                  >
                    <span>{t.emoji}</span>
        ...
        <motion.div
          ...
          className="rounded-3xl p-5 sm:p-6 bg-(--bg-card) border border-(--border-glass) space-y-4 shadow-xs"
        >
          <div className="flex items-center justify-between border-b border-(--border-glass) pb-3">
            <div className="flex items-center gap-2 text-sm font-black text-(--text-primary) uppercase tracking-wider">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
            <div className="p-4 rounded-2xl bg-black/3 dark:bg-white/5 border border-(--border-glass) space-y-2">
            ...
            <div className="p-4 rounded-2xl bg-black/3 dark:bg-white/5 border border-(--border-glass) space-y-2">
              ...
              <a
                ...
                className="inline-flex items-center gap-1 text-(--text-primary) font-bold bg-black/5 dark:bg-white/10 hover:bg-black/10 dark:hover:bg-white/15 px-2.5 py-1.5 rounded-lg text-[11px] transition"
              >
