...
export default function CheckoutPage() {
  ...
  // Auto-detect student account from logged-in session or stored profile
  useEffect(() => {
    if (profile?.full_name && !studentName) {
      setStudentName(profile.full_name);
    } else if (user?.user_metadata?.full_name && !studentName) {
      setStudentName(user.user_metadata.full_name);
    }

    if (profile?.prn && !studentPrn) {
      setStudentPrn(profile.prn);
    } else if (user?.user_metadata?.prn && !studentPrn) {
      setStudentPrn(user.user_metadata.prn);
    } else if (typeof window !== 'undefined') {
      const savedPrn = localStorage.getItem('foodline_last_prn');
      if (savedPrn && !studentPrn) setStudentPrn(savedPrn);
    ...
    return (
      <PageTransition className="min-h-screen bg-(--bg-canvas) text-(--text-primary) pb-24 transition-colors duration-500">
        ...
        <main className="max-w-xl mx-auto px-4 pt-20 text-center">
          <SpotlightCard className="p-8 sm:p-10 rounded-[2.5rem] text-center border-dashed border-(--border-glass)">
            <EmptyCartIllustration size={180} className="mx-auto mb-2" />
            <h1 className="text-2xl sm:text-3xl font-black text-(--text-primary) mb-2">Your Tray is Empty</h1>
  ...
  return (
    <PageTransition className="min-h-screen bg-(--bg-canvas) text-(--text-primary) pb-40 transition-colors duration-500">
      <Navbar />
      ...
      <main className="max-w-6xl mx-auto px-4 pt-6">
        ...
        {/* Stepper Header */}
        <div className="glass-card-heavy rounded-4xl p-6 mb-8 border border-(--border-glass) shadow-xl">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-7 space-y-6">
                  ...
                  return (
                    <div key={item.id} className="pt-3.5 first:pt-0 flex flex-wrap sm:flex-nowrap items-center justify-between gap-3">
                      <div className="min-w-0 flex-1">
                        <h4 className="text-sm font-extrabold text-(--text-primary) truncate">{item.name}</h4>
                        <div className="flex items-center gap-2">
                          <span className="text-[11px] text-(--text-secondary) font-bold">₹{item.price} each</span>
                      ...
                      <div className="flex items-center gap-3 shrink-0 ml-auto">
                        <div className="flex items-center gap-1.5 bg-black/5 dark:bg-white/5 border border-(--border-glass) rounded-xl p-1 shadow-inner">
                          <button
                            type="button"
                        ...
                        <span className="text-sm font-black text-(--text-primary) w-14 text-right">
            ...
            <SpotlightCard className="p-5 sm:p-6 md:p-7 relative overflow-hidden">
              <div className="flex items-center justify-between gap-4 mb-4 flex-wrap">
                <div className="flex items-center gap-3">
                  <div className="w-11 h-11 rounded-2xl bg-accent-amber/15 border border-accent-amber/30 flex items-center justify-center text-accent-amber shadow-inner shrink-0">
                    <Clock size={22} strokeWidth={2.4} />
                  </div>
                  <div>
                    <h2 className="text-lg font-black text-(--text-primary) leading-tight">Campus Break Pickup Slot</h2>
                    <p className="text-xs text-(--text-secondary) font-medium mt-0.5">Reserve your 15-min express collection window to avoid queue delays</p>
                  </div>
                </div>
              ...
              <div className="flex items-center justify-between gap-2 p-3 rounded-2xl bg-black/3 dark:bg-white/4 border border-(--border-glass) mb-4 text-xs">
                <div className="flex items-center gap-2">
              ...
              <div className="grid grid-cols-2 gap-2 p-1 bg-black/4 dark:bg-white/6 rounded-2xl mb-4 border border-(--border-glass)">
                <button
                  type="button"
                    ...
                    <motion.button
                      ...
                      className={`w-full text-left p-4 rounded-2xl border transition-all duration-200 ${
                        isSelected
                          ...
                          : isClosed
                          ? 'bg-black/2 dark:bg-white/2 border-(--border-glass) opacity-40 cursor-not-allowed'
                          : 'bg-black/3 dark:bg-white/5 border-(--border-glass) hover:border-accent-orange/40 hover:bg-black/6 dark:hover:bg-white/10 cursor-pointer'
                      }`}
                    >
                          ...
                          <div>
                            <span
                              className={`text-sm font-extrabold ${
                                isClosed ? 'text-zinc-400 dark:text-zinc-500 line-through' : 'text-(--text-primary)'
                              }`}
                            >
                              {slot.label}
                            </span>
          ...
          <div className="lg:col-span-5 space-y-6">
            <SpotlightCard spotlightColor="var(--accent-teal-glow, rgba(0, 212, 170, 0.25))" className="p-6 md:p-7">
              ...
              <div className="flex items-center justify-between gap-3 mb-6 p-3.5 rounded-2xl bg-black/5 dark:bg-black/50 border border-(--border-glass)">
                <div className="flex items-center gap-2.5">
                  ...
                  <div>
                    <span className="text-xs font-black text-(--text-primary) block">⚡ DirectPay UPI</span>
                    <span className="text-[10px] text-(--text-secondary) font-medium">100% Online Bank Settlement</span>
                  </div>
                </div>
              ...
              <div>
                <div className="flex items-center justify-between gap-3 mb-2">
                  <div className="flex items-center gap-2">
                    ...
                    <h2 className="text-lg font-black text-(--text-primary)">Direct UPI Payment</h2>
                  </div>
                  <span className="text-[10px] font-black uppercase tracking-wider text-accent-teal bg-accent-teal/10 border border-accent-teal/30 px-2.5 py-1 rounded-full">
                    Instant Fast-Pass
                  </span>
                ...
                <motion.button
                  ...
                  onClick={copyUpiId}
                  type="button"
                  className="w-full mt-4 py-2.5 rounded-xl bg-black/5 dark:bg-white/5 border border-(--border-glass) text-xs font-bold text-(--text-secondary) hover:text-(--text-primary) hover:bg-black/10 dark:hover:bg-white/10 transition cursor-pointer flex items-center justify-center gap-2"
                >
                  {copiedUpi ? (
                    <>
              ...
              <form onSubmit={handlePlaceOrder} className="mt-6 space-y-4">
                <div>
                  <label className="block text-[11px] font-black text-(--text-secondary) uppercase tracking-wider mb-2">
                  ...
                  <input
                    ...
                    className="w-full bg-(--bg-card) border border-(--border-glass) rounded-2xl px-5 py-4 text-sm text-(--text-primary) font-mono tracking-widest placeholder-(--text-muted) focus:outline-none focus:border-accent-orange focus:ring-2 focus:ring-accent-orange/20 transition-all shadow-inner"
                  />
                  <div className="flex items-center justify-between mt-1.5">
                    <span className="text-[10px] text-(--text-muted) font-medium">Found in your UPI transaction receipt</span>
                    <span className={`text-[11px] font-mono font-bold ${utrNumber.length === 12 ? 'text-accent-teal' : 'text-(--text-muted)'}`}>
                      {utrNumber.length}/12
                    </span>
                  </div>
                </div>

                {/* Auto-Detected Account Banner */}
                {(studentName || studentPrn) && (
                  <motion.div
                    initial={{ opacity: 0, y: -4 }}
                    animate={{ opacity: 1, y: 0 }}
                    className="p-3 rounded-2xl bg-emerald-500/10 border border-emerald-500/30 flex items-center justify-between text-xs text-emerald-600 dark:text-emerald-400 shadow-xs"
                  >
                    <div className="flex items-center gap-2.5">
                      <CheckCircle2 size={18} className="text-emerald-500 shrink-0" />
                      <div>
                        <span className="font-black block text-[10px] uppercase tracking-wider text-emerald-500">
                          Student Account Auto-Detected
                        </span>
                        <span className="text-xs font-bold text-(--text-primary)">
                          {studentName || 'Campus Student'} • <span className="font-mono text-emerald-600 dark:text-emerald-400">{studentPrn}</span>
                        </span>
                      </div>
                    </div>
                    <span className="text-[10px] uppercase font-mono px-2.5 py-0.5 rounded-full bg-emerald-500/20 text-emerald-600 dark:text-emerald-400 font-black">
                      Verified
                    </span>
                  </motion.div>
                )}
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className="block text-[10px] font-bold text-(--text-secondary) uppercase tracking-wider mb-1">Student Name</label>
                    <input
                      type="text"
                  ...
                  <div>
                    <label className="block text-[10px] font-bold text-(--text-secondary) uppercase tracking-wider mb-1">PRN / Roll No.</label>
                    <input
                      type="text"
                ...
                <div className="bg-black/3 dark:bg-white/5 border border-(--border-glass) rounded-2xl p-4 my-4 space-y-2.5 shadow-inner">
                  <div className="text-[10px] font-black uppercase text-(--text-secondary) tracking-wider flex items-center justify-between">
                  ...
                  <div className="pt-2.5 border-t border-(--border-glass) flex justify-between text-sm font-black text-(--text-primary)">
                    <span>Total Amount Payable</span>
                    <span className="text-accent-orange font-mono text-xl font-black">₹{finalPayable.toFixed(2)}</span>
                  </div>
                  <div className="text-[10px] text-(--text-secondary) font-medium pt-1.5 border-t border-(--border-glass) flex items-start gap-1.5 leading-relaxed">
                    <span className="text-accent-teal text-xs">⚡</span>
                    <span>
                      <strong className="text-(--text-primary) font-bold">100% Direct Settlement:</strong> 100% of your payment lands directly in Cafe @7 merchant bank account (<code className="text-accent-teal font-mono">{upiId}</code>) with zero food markup.
                    </span>
                  </div>
                ...
                <div className="p-3.5 rounded-2xl bg-black/3 dark:bg-white/5 border border-(--border-glass) space-y-2 mb-4">
                  ...
                  <div className="text-[9px] text-(--text-muted) flex items-center gap-2 pl-6">
                  ...
                  <motion.button
                    ...
                    className={`w-full py-4.5 rounded-2xl font-black text-sm md:text-base transition-all cursor-pointer flex items-center justify-center gap-2 shadow-2xl mt-4 mb-4 ${
                      isSubmitting || utrNumber.length !== 12 || !dpdpConsent
                        ? 'bg-black/10 dark:bg-zinc-800 text-(--text-muted) cursor-not-allowed border border-(--border-glass)'
        ...
        {celebrationData && (
          ...
          >
            ...
            <motion.div
              ...
              className="relative max-w-md w-full rounded-[2.5rem] border-2 border-accent-teal/50 p-6 sm:p-8 text-center bg-(--bg-card) text-(--text-primary) shadow-[0_0_60px_rgba(0,212,170,0.3)] overflow-hidden"
            >
              ...
              <p className="text-xs sm:text-sm text-(--text-secondary) mb-6 font-medium">
                Your order is locked for <strong className="text-(--text-primary)">{celebrationData.slotLabel}</strong>. Kitchen ticket dispatched!
              </p>

              {/* OTP Preview Card */}
              {celebrationData.pickupOtp && (
                <div className="p-4 rounded-2xl bg-black/5 dark:bg-black/60 border border-(--border-glass) mb-6 shadow-inner">
              ...
              <div className="space-y-3">
                ...
                <div className="text-[11px] text-(--text-muted) font-medium flex items-center justify-center gap-1.5">
                  ...
                  <span>Auto-navigating to pass in <strong className="text-(--text-primary) font-mono">{countdown}s</strong>...</span>
                </div>
              </div>
            </motion.div>
          </motion.div>
