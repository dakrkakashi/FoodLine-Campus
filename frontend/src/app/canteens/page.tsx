  ...
  return (
    <PageTransition className="min-h-screen bg-(--bg-canvas) text-(--text-primary) pb-32 relative overflow-hidden transition-colors duration-500">
      ...
      <main className="max-w-6xl mx-auto px-4 pt-6 relative z-10">
        ...
        <div className="glass-card-heavy rounded-[2.5rem] p-6 md:p-8 mb-8 border border-(--border-glass) shadow-xl flex flex-col md:flex-row md:items-center justify-between gap-6 relative overflow-hidden">
          ...
          <div className="relative z-10">
            ...
            <h1 className="text-2xl md:text-4xl font-black text-(--text-primary) tracking-tight flex items-center gap-3">
              <span>{selectedCampus.name}</span>
            </h1>
            <p className="text-xs md:text-sm text-(--text-secondary) mt-1 flex items-center gap-1.5 font-medium">
          ...
          <div className="relative z-10 flex items-center gap-3 shrink-0">
            <Link href="/select-campus">
              <motion.button
                ...
                whileTap={{ scale: 0.96 }}
                className="px-5 py-3 rounded-2xl bg-black/5 dark:bg-white/10 hover:bg-black/10 dark:hover:bg-white/15 border border-(--border-glass) text-(--text-primary) font-bold text-xs flex items-center gap-2 cursor-pointer transition shadow-sm"
              >
          ...
          <div className="flex flex-col sm:flex-row items-center gap-3 justify-between">
            <div className="relative w-full sm:max-w-md">
              ...
              <input
                ...
                className="w-full bg-(--bg-card) border border-(--border-glass) rounded-2xl pl-11 pr-4 py-3.5 text-xs sm:text-sm text-(--text-primary) placeholder-(--text-muted) focus:outline-none focus:border-accent-orange transition-all shadow-inner"
              />
            </div>
              ...
              {[
                ...
                <button
                  ...
                  className={`px-4 py-2.5 rounded-2xl text-xs font-black tracking-wide shrink-0 transition-all cursor-pointer ${
                    filterTag === pill.id
                      ? 'bg-linear-to-r from-accent-orange to-accent-amber text-white dark:text-black shadow-lg shadow-accent-orange/25'
                      : 'bg-(--bg-card) border border-(--border-glass) text-(--text-secondary) hover:text-(--text-primary)'
                  }`}
                >
                  {pill.label}
                </button>
        ...
        {filteredCanteens.length === 0 ? (
          <SpotlightCard className="p-10 rounded-[2.5rem] text-center border-dashed border-(--border-glass)">
            ...
            <h3 className="text-lg font-bold text-(--text-primary) mb-1">No canteens matched your search</h3>
            <p className="text-xs text-(--text-secondary) mb-4">Try clearing your search query or view all available campus outlets.</p>
            <button
              onClick={() => {
                setSearchQuery('');
                setFilterTag('ALL');
              }}
              className="px-5 py-2.5 rounded-xl bg-black/5 dark:bg-white/10 text-(--text-primary) text-xs font-bold hover:bg-black/10 dark:hover:bg-white/15 transition cursor-pointer"
            >
              Reset Filters
            </button>
          </SpotlightCard>
              ...
              return (
                ...
                >
                  <SpotlightCard
                    ...
                    className={`h-full p-6 md:p-7 rounded-[2.5rem] border border-(--border-glass) hover:border-accent-orange/50 hover:shadow-[0_0_35px_var(--accent-orange-glow)] transition-all duration-300 bg-(--bg-card) backdrop-blur-xl flex flex-col justify-between group cursor-pointer shadow-sm dark:shadow-none ${
                  ...
                  >
                    <div>
                      ...
                      {/* Title & Tagline */}
                      <h2 className="text-xl md:text-2xl font-black text-(--text-primary) group-hover:text-accent-amber transition-colors leading-tight mb-1.5">
                        {canteen.name}
                      </h2>
                      <p className="text-xs font-bold text-accent-teal mb-2">{canteen.tagline}</p>
                      <p className="text-xs text-(--text-secondary) flex items-start gap-1.5 mb-4 leading-relaxed">
                        <MapPin size={13} className="text-accent-orange shrink-0 mt-0.5" />
                        <span>{canteen.location}</span>
                      </p>
                    </div>

                    {/* Footer Metrics & Open Action */}
                    <div className="pt-4 border-t border-(--border-glass) flex items-center justify-between gap-4 mt-4">
                      <div>
                        <span className="text-[10px] text-(--text-muted) uppercase tracking-wider font-bold block">Live Inventory</span>
                      ...
                      <motion.button
                        ...
                        whileTap={{ scale: 0.95 }}
                        disabled={!isOpen}
                        className="px-5 py-3 rounded-2xl bg-linear-to-r from-accent-orange via-accent-amber to-accent-amber text-white dark:text-black font-black text-xs shadow-xl shadow-accent-orange/30 flex items-center gap-2 cursor-pointer border border-white/20 uppercase tracking-wider"
                      >
