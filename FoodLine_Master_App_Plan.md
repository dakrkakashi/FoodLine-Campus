...
```mermaid
graph TD
    SOGR[1. The Science of Getting Rich: Creation over Competition & 10x Use Value] --> MasterPlan[FoodLine Master Execution Plan]
    RDGTI[2. Guide to Investing: The B-I Triangle, 10 Controls & Inside Asset Building] --> MasterPlan
    WWBP[3. Writing Winning Business Plans: Bottom-Up Math, Legal Moats & Clear Exit] --> MasterPlan
```
...
```mermaid
graph LR
    Orders[500 Orders / Day] --> AOV[Average Order Value: ₹55]
    AOV --> DailyGMV[Daily GMV: ₹27,500 / Campus]
    DailyGMV --> AnnualGMV[Annual GMV: ₹68.75 Lakhs / Campus]
    AnnualGMV --> Scale50[50 Campuses: ₹34.37 Crores GMV]
```
...
sequenceDiagram
    autonumber
    actor Student
    participant App as FoodLine Client
    participant Server as Next.js API Edge
    participant DB as Supabase PostgreSQL
    actor Vendor as Cafe @7 Merchant Account

    Student->>App: Tap 'Proceed to Pay' (Total: ₹70)
    App->>Server: POST /api/orders/create (Order Token: #FL-8492)
    Server->>DB: Reserve Slot 1 Capacity (Status: PENDING_PAYMENT)
    Server-->>App: Display Cafe @7 QR Standee + UPI ID: 9960091371@slc
    
    Note over Student,Vendor: Student switches to GPay/PhonePe and pays ₹70 directly
    Vendor-->>Vendor: 100% of ₹70 received directly in Canteen Bank (0% PG fee)
    
    Student->>App: Enters 12-Digit Bank UTR (e.g. 423819028471)
    App->>Server: POST /api/payments/verify-utr
    Server->>DB: SELECT * FROM utr_transactions WHERE utr_number = '423819028471'
    
    alt UTR is Unique & Fresh
        Server->>DB: INSERT INTO utr_transactions (utr, order_id, amount)
        Server->>DB: UPDATE orders SET status = 'VERIFIED'
        Server->>Server: Dispatch SSE Event ('order:verified')
        Server-->>App: Return Verified Optical QR Pass + PIN: 4921
    else UTR Already Exists (Replay Attack)
        Server-->>App: 400 Bad Request: "UTR already redeemed for order #FL-8102"
    end
```
...
```mermaid
graph TD
    subgraph What Student Pays
        Cash[Cash Value: ₹55 for Masala Dosa + Tea]
    end

    subgraph What Student Receives in Use Value
        Time[12+ Minutes of Recess Saved = Value ₹200]
        Comfort[Zero Exhausting Queue Standing = Value ₹100]
        Academic[Arriving on Time to Lab/Lectures = Value ₹150]
        Fresh[Guaranteed Hot & Fresh Food = Value ₹100]
    end

    Cash -->|Transformed Into| Time
    Cash -->|Transformed Into| Comfort
    Cash -->|Transformed Into| Academic
    Cash -->|Transformed Into| Fresh
```
...
```mermaid
graph LR
    P1[Phase 1: Pilot Saturation Sanjivani Cafe @7] --> P2[Phase 2: City Cluster 50 Campuses in MH]
    P2 --> P3[Phase 3: National Scale 5,000+ Campuses Pan-India]
    P3 --> Exit[Strategic Acquisition 5-8x ARR by Swiggy / Zomato / DotPe]
```
